...
from model.help_funcs import (
    ...
    parse_flexible_classification,
    parse_flexible_molecule,
    parse_flexible_caption,
...
class Blip2Stage3(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        # 런마다 unique한 샘플 저장 폴더 생성 (타임스탬프 기반)
    ...
    def transfer_batch_to_device(self, batch, device, dataloader_idx):
        """
        문자열 리스트 등 non-tensor 데이터는 GPU로 이동하지 않고 그대로 유지
        BatchEncoding (transformers tokenizer output)도 내부에 문자열이 있을 수 있으므로
        dict처럼 개별 처리
        """
        from transformers.tokenization_utils_base import BatchEncoding

        # BatchEncoding 또는 dict인 경우 개별 항목 처리
        if isinstance(batch, (dict, BatchEncoding)):
            ...
            # BatchEncoding인 경우 BatchEncoding으로 반환
    ...
    def save_sample_realtime(
        ...
        """실시간으로 샘플을 파일에 append 모드로 저장"""
        # EOS 토큰 정리 (</s>만 유지)
        clean_prediction = prediction
        for eos_token in ['<|end_of_text|>', '<|endoftext|>', '[EOS]', '<eos>', '<|eot_id|>']:
            clean_prediction = clean_prediction.replace(eos_token, '')

        # Benchmark 모드이고 reaction/text2mol task인 경우 SMILES → SELFIES 변환
        is_benchmark = getattr(self.args, 'benchmark', False)
        is_mol_output_task = task in REACTION_BENCHMARKS or task in TEXT2MOL_BENCHMARKS

        prediction_selfies = None
        if is_benchmark and is_mol_output_task:
            # parse_flexible_molecule: SMILES/SELFIES를 SELFIES로 변환
            prediction_selfies = parse_flexible_molecule(clean_prediction)
            if prediction_selfies:
                clean_prediction = f"<SELFIES>{prediction_selfies}</SELFIES>"
        ...
        instance = {
            ...
            "target": target,
            "prompt": prompt,
            "input_mol_strings": input_mol_string,
            "batch_idx": batch_idx,
            "rank": self.global_rank,
        }
        if task in CLASSIFICATION_BENCHMARKS and prob is not None:
            instance["prob"] = prob

        # 첫 저장 시점에 samples 폴더 생성
        if self.samples_dir is None:
            self.samples_dir = os.path.join(self.logger.log_dir, f"samples_{self.samples_subdir}")
            os.makedirs(self.samples_dir, exist_ok=True)
            print(f"[Rank {self.global_rank}] Created samples directory: {self.samples_dir}")

        # Task별로 개별 파일에 저장 (append mode)
        safe_task_name = task.replace("/", "_").replace(" ", "_")
        filename = f"{self.args.mode}-rank{self.global_rank}-{safe_task_name}-samples.jsonl"
    ...
    def evaluation_step(self, batch, batch_idx, dataloader_idx, mode="val"):
        ...
        # Use prompt_input_ids (inference prompt without answer) instead of input_ids (full sequence with answer)
        prompts = self.blip2model.llm_tokenizer.batch_decode(
            prompt_input_ids, skip_special_tokens=False
        )
        predictions = [
       ...
       # ================= [수정된 전체 출력 코드 시작] =================
        ...
        if self.args.debug:
            ...
            debug_log_path = os.path.join(debug_log_dir, f"debug_rank{self.global_rank}.log")
            ...
            for k, task_name in enumerate(tasks):
                ...
                if task_base in CLASSIFICATION_BENCHMARKS:
                    raw_lower = raw_pred.lower().strip()
                    if raw_lower.startswith('true'):
                        parsed_pred = "True"
                    elif raw_lower.startswith('false'):
                        parsed_pred = "False"
                    elif raw_lower.startswith('yes'):
                        parsed_pred = "True"
                    ...
                    parsed_pred = parsed if parsed else "PARSE FAILED"
                elif task_base in TEXT2MOL_BENCHMARKS or task_base in REACTION_BENCHMARKS:
                    parsed = parse_flexible_molecule(raw_pred)
                    if parsed:
                        try:
                            # parsed가 SELFIES면 SMILES로 변환, 이미 SMILES면 그대로 사용
                            if parsed.startswith('[') and ']' in parsed:
                                # SELFIES 형식
                                smiles = selfies.decoder(parsed)
                            else:
                                # 이미 SMILES
                                smiles = parsed
                            ...
                            if mol:
                                ...
                                # SMILES로 표시 (SELFIES 변환 불필요)
                                parsed_pred = canonical_smiles
                            else:
                                parsed_pred = "PARSE FAILED (invalid molecule)"
                        except:
                            parsed_pred = "PARSE FAILED (conversion error)"
                    else:
                        parsed_pred = "PARSE FAILED (no molecule found)"
                else:
                    parsed_pred = display_pred

                # TARGET을 SMILES로 변환해서 표시 (비교 용이)
                target_display = targets[k]
                if task_base in TEXT2MOL_BENCHMARKS or task_base in REACTION_BENCHMARKS:
                    try:
                        import re as re_module
                        target_match = re_module.search(r'<SELFIES>(.*?)</SELFIES>', targets[k])
                        if target_match:
                            target_selfies = target_match.group(1).replace(' ', '')
                            target_smiles = selfies.decoder(target_selfies)
                            target_mol = Chem.MolFromSmiles(target_smiles)
                            if target_mol:
                                target_display = Chem.CanonSmiles(target_smiles)
                    except:
                        pass  # 변환 실패시 원본 유지

                # 5가지 출력 내용 구성 (RAW PREDICTION 추가)
                raw_prompt = batch["raw_prompt_text"][k] if "raw_prompt_text" in batch and k < len(batch["raw_prompt_text"]) else "(not available)"
                log_content = f"""
{'='*80}
[RAW PROMPT]
{raw_prompt}
...
"""
    ...
    def on_evaluation_epoch_end(self, mode="val") -> None:
        ...
        # Use flexible parsing for benchmark mode (e.g., Galactica, GPT-4)
        is_benchmark = getattr(self.args, 'benchmark', False)
        include_failed_as_zero = getattr(self.args, 'include_failed_as_zero', False)

        evaluation_results, failed_cases, converted_predictions = per_device_evaluate(
            ...
            flexible_parsing=is_benchmark,
            include_failed_as_zero=include_failed_as_zero,
        )

        # [DEBUG] 평가 직전 데이터 출력 (converted_predictions 사용)
        if getattr(self.args, 'debug', False):
            num_samples = min(10, len(converted_predictions))
            print(f"\n{'='*80}")
            print(f"[DEBUG] 평가 직전 데이터 샘플 (converted_predictions) - {num_samples}개")
            print(f"{'='*80}")
            for i in range(num_samples):
                print(f"\n--- Sample {i+1} ---")
                print(f"[TASK] {self.list_logs['tasks'][i]}")
