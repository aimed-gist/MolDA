...
# for H100 GPUs - use high precision (TF32)
torch.set_float32_matmul_precision(
    "high"
)  # high (tensorfloat32) is optimal for H100
...
def main(cfg):
    ...
    # Debug 모드일 때 로그 파일 설정
    import sys
    from datetime import datetime

    if getattr(cfg, 'debug', False):
        log_dir = os.path.join(cfg.logging_dir, cfg.filename)
        os.makedirs(log_dir, exist_ok=True)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        log_file_path = os.path.join(log_dir, f"debug_{timestamp}.log")

        # Tee 클래스: stdout을 파일과 콘솔 둘 다에 출력
        class Tee:
            def __init__(self, filepath, stream=None):
                self.terminal = stream if stream else sys.stdout
        ...
        sys.stderr = Tee(os.path.join(log_dir, f"debug_{timestamp}_stderr.log"), sys.stderr)
    ...
    if cfg.mode in {"ft"}:
        ...
        if cfg.ckpt_path is not None:
            ...
            print("No checkpoint provided - using model as initialized (benchmark mode)")
        outputs = trainer.test(model, datamodule=dm)
